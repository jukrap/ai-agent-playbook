...
Version 0.5.7 keeps the resumable local execution loop and the human-facing Forge coordination model introduced in 0.5.5. Fine-grained tasks stay in the local ledger, while roadmap and delivery-group issues, Projects, Views, milestones, and reviewable PRs present the shared work. Interrupted supersede migrations now recover exact marker-owned issues, close them before comment and hierarchy mutations can invalidate the reviewed snapshot, and remove obsolete task cards from the managed Project. Missing Projects authorization still stops before the first write with direct recovery commands, and newly managed Project fields use neutral names while retaining legacy aliases for compatibility.
...
See the [0.5.7 Forge reconcile recovery change note](docs/changes/forge-reconcile-recovery-0.5.7.md) for interrupted supersede recovery and Project item cleanup. The [0.5.6 Forge permission guidance change note](docs/changes/forge-permission-guidance-0.5.6.md) remains the authorization and Project-field compatibility reference, and the [0.5.5 human-centered Forge coordination change note](docs/changes/forge-human-coordination-0.5.5.md) remains the presentation migration reference.
