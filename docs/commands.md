...
`automation resume --reset-attempts` is an explicit retry-budget decision for blocked tasks. It preserves every historical ledger event and continues with the next monotonic attempt serial instead of reusing `attempt:1` or another prior event ID. Inspect the prior failure evidence before resetting; use a new run only when the plan or execution baseline genuinely requires a separate history.
...
Generated GitHub/Gitea workflows can initialize a fresh runner when repository variable `AAPB_AUTOMATION_PLAN` names a committed, approved plan sidecar. After restoring the run cache, they pass that value as the quoted environment expansion `"$AAPB_AUTOMATION_PLAN"` to an idempotent `automation start`, then run one tick. Both commands pin the exact AAPB version from the package's release metadata, and validation rejects generator or copy-template version drift. If the variable is absent, an existing run must already be available from the checkout or restored cache. Reusing the same `planId` reuses the same default run and does not rewrite it with changed plan contents. Existing workflow files with different content remain preserved and need an explicitly reviewed package-pin update when upgrading.
