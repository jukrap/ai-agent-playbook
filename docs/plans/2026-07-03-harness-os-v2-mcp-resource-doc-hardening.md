...
**Goal:** Make the expanded harness easier for AI apps to discover and use by exposing structured MCP resources, tightening `.ai-agent-playbook` entry guidance, and aligning README/MCP docs with the current implementation.
...
**Architecture:** Keep MCP read-only by default. Use resources for stable discovery packets, prompts for reusable review briefs, read-only tools for evidence gathering, and opt-in write tools only for bounded `.ai-agent-playbook` or runtime artifacts. Keep project source writes outside MCP.
...
- Update `.ai-agent-playbook` template entry files so agents start from `START_HERE.md`, `CURRENT.md`, questions, relevant memory, contracts, maps, and workflow recipes.
...
- [x] Add `ai-agent-playbook://adapters` for Codex, Codex App, Claude Code, and MCP setup discovery.
- [x] Add `ai-agent-playbook://playbook-layout-v2` for `.ai-agent-playbook` read order and runtime-vs-memory rules.
- [x] Add `ai-agent-playbook://mcp-permission-model` for default resources, permission tiers, and opt-in write tools.
- [x] Update MCP tests for the new resources.
- [x] Update `.ai-agent-playbook` `START_HERE.md` and `SKILLS.md` templates.
...
- Add a `.ai-agent-playbook` usage smoke test that bootstraps a fixture and checks that `START_HERE.md`, `SKILLS.md`, workflow recipes, runtime, and knowledge files remain coherent.
