...
4. `context/`: path-scoped facts, reading hints, and assumptions to avoid.
5. `maps/doc-map.md`: the guide to public docs and project-memory locations.
6. `maps/`: repository, runtime, route, API, data, or risk maps.
7. `runbooks/`: commands and operational procedures.
8. `contracts/`: active or pending business rules and invariants.
9. `plans/`: active implementation plans only.
10. `runs/`: in-progress evidence ledger for one task.
11. `worklogs/`: detailed history and monthly summaries.
12. `guides/`: reusable support guides, including `harness-migration.md` for projects that already have agent docs or another harness and `runtime-roadmap.md` for optional hook-layer decisions.
...
4. `.ai-playbook/CURRENT.md`, context, maps, runbooks, contracts, and decisions.
5. Runs, worklogs, and archived notes.

Runs are in-progress evidence. Worklogs are history. Promote facts that remain current into `CURRENT.md`, `context/`, `maps/`, `runbooks/`, `contracts/`, or `decisions/`.
...
Put larger material in the matching subfolder. Use `context/` for path-scoped reading hints, `maps/` for structure, `runbooks/` for commands, `contracts/` for business rules and invariants, `decisions/` for durable choices, `plans/` for active execution, `runs/` for evidence during the work, and `worklogs/` for detailed history.
