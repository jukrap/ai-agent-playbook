...
The templates invoke the exact `ai-agent-playbook` release with `npx` and disable package install scripts. If the runner cannot reach the package registry, install the pinned CLI in the runner image or repository and replace that line with the equivalent local `aapb automation tick` command.
