...
- `.ai-agent-playbook/`: agent-facing project memory, maps, runbooks, plans, decisions, and worklogs.
...
- Keep current project truth in `.ai-agent-playbook/CURRENT.md`.
...
Decide whether `.ai-agent-playbook/` is committed or local-only. If local-only, add it to `.gitignore` before writing private project facts.
