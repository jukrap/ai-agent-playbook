...
- Keep active execution plans in `workflows/plans/`.
- Keep in-progress evidence and criteria in `workflows/runs/`.
- Keep detailed reasoning and recovery history in `workflows/worklogs/YYYY-MM/`.
- Keep business rules and invariants in `memory/contracts/`.
- Keep durable structure facts in `memory/maps/`.
- Keep repeatable commands and operational procedures in `workflows/runbooks/`.
- Keep accepted choices in `memory/decisions/`.
