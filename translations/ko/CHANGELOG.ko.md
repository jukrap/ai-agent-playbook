...
- 읽기 전용 `ast search`로 코드 구조 검색을 복원하고, 선택 의존성 ast-grep 엔진을 호출할 때만 로딩합니다.
- MCP를 `--with-ast`로 시작할 때만 `aapb_ast_search`를 추가합니다. 기본 기록 도구 네 개와 기존 스킬 프로필은 유지합니다.
- 소스 선택·파싱·코드 조각·결과 페이지에 상한을 두고, 불완전한 검색 범위를 표시하며 원본이 바뀐 커서와 링크 경로를 거부합니다.
- 언어 선택, 셸 따옴표, npm 엔진 설치와 MCP의 일회성 활성화 설정을 한영 문서에 설명합니다.
