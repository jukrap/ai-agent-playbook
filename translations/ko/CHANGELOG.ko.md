...
- 프로젝트 기록, 선택형 스킬 여섯 개, 중립 프로젝트 지침과 선택형 읽기 전용 기록 MCP를 제공합니다.
- 기본 명령 `ai-agent-playbook`과 축약 명령 `aapb`를 제공하며 프로젝트 경로를 생략하면 현재 폴더를 사용합니다.
- 설치·이전에서 사용자 편집을 보존하고 복구 일지를 제공하며 Forge 협업은 명시적으로 적용합니다.
- 이전 실행기·예약기·중복 분석 명령을 종료하고 0.5.11 버전 고정 복구 방법을 안내합니다.
- npm 패키지에 영어·한국어 사용법, 이전, 아키텍처와 릴리스 문서를 포함합니다.

[릴리스 안내](docs/release-1.0.0.ko.md)

## 1.0.0-next.3

- 기술별 루트 지침을 중립 템플릿 하나로 정리하고 분야별 계약은 선택 참고 자료에 보존했으며 프로젝트별 아키텍처 결정 방법을 설명했습니다.
- 개인 지침의 유용한 내용을 보강하고 오래된 부트스트랩 참조를 바로잡았습니다. 자동 설치 동작은 유지합니다.

- 기본 명령 `ai-agent-playbook`을 추가하고 같은 기능의 축약 명령 `aapb`를 유지했습니다.
- 프로젝트 인자를 생략하면 현재 폴더를 사용하는 기존 동작을 문서화하고 검증했습니다.
- 영어·한국어 명령어 가이드에 전체 명령 조합, 옵션의 뜻, 쓰기 여부, 복구 예시를 보강했습니다.
- 구현 설계 기록을 사용자용 1.0 개편 이유와 0.5.11 사용 안내로 바꾸고 스킬 선택과 MCP 검증을 구분해 설명했습니다.
