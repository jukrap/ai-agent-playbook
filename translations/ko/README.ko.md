...
| MCP 도구         | AI 앱이 호출할 수 있는 로컬 읽기 전용 tool, resource, prompt입니다. catalog, adapter 지원과 readiness, reference adoption, playbook layout, 권한 모델, index search, write-gate preview, context, operator check/search/research, contract, image diff, AST 검색, 함수 본문 중복 단서, TypeScript/JavaScript 분석을 다룹니다. | `src/`             |
...
사용 중인 AI 앱이 MCP를 지원한다면 `npx ai-agent-playbook mcp` 같은 로컬 서버 명령을 등록할 수 있습니다. 그러면 명령어를 전부 외우지 않아도 AI에게 playbook context 점검, capability/skill/workflow/adapter/reference-adoption resource 읽기, operator search, deep local analysis를 자연어로 요청할 수 있습니다. MCP 도구는 기본적으로 읽기 전용입니다.
...
- 프로젝트와 문서: bootstrap, onboarding, project docs, ADR/spec handoff, PRD/scope, issue planning, release notes, documentation package.
- 전달과 검증: Git/worklog, verification strategy, CI gate, flaky test, fixture/test data, eval harness.
- AI 하네스: MCP surface, context/memory engineering, agent orchestration, skill-pack governance, runtime index/cache, capability witness, fact gate.
- 아키텍처와 백엔드: boundary review, FSD/feature slice, domain model, monorepo package, API contract, backend change safety, connector, server-rendered flow.
- 데이터와 데이터베이스: analytics, lineage, migration, retrieval, source registry, schema migration, query performance, data integrity.
- DevOps와 릴리스: container, deployment, package publish, observability incident, release readiness.
- 디자인과 프론트엔드: design direction, brand identity, reference analysis, image/Figma handoff, UI polish, accessibility, state/data flow, visual regression, 3D/interactive media.
- 모바일: native release, device permission, offline sync, WebView bridge.
- 보안과 컴플라이언스: security review, auth/access control, dependency supply chain, license/notice, compliance gate.
- 레거시: hidden coupling, old web stacks, server-rendered systems, WebView hybrids, IE/ActiveX compatibility, enterprise database-heavy flows.
