...
| MCP 도구 | 기본 기록 도구 네 개와 선택형 코드 구조 검색 | `src/` |
...
[AST 코드 구조 검색](docs/ast-search.ko.md)은 CLI로 사용하거나 `--with-ast`로 `aapb_ast_search` 도구를 추가할 수 있습니다. 기본 기록 도구 네 개와 설치 스킬 수는 그대로입니다.
