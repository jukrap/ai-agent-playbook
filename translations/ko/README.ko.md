...
이 저장소는 slash command 묶음, Codex plugin, 자동 실행 에이전트가 아닙니다. 기본 모델은 operator-in-the-loop입니다. 사람 또는 에이전트가 CLI를 명시적으로 실행하고, dry-run 결과를 검토한 뒤 파일을 쓸지 선택합니다.
...
패키지는 [`ai-agent-playbook`](https://www.npmjs.com/package/ai-agent-playbook)으로 publish되어 있습니다. 한 번만 실행할 때는 `npx`를 쓰고, 짧은 `ai-playbook` 명령이 필요하면 global install을 사용합니다.

전체 설정, 업데이트, 정리 가이드는 [설치, 업데이트, 삭제](docs/installation.ko.md)를 봅니다.

### 1. CLI 실행 방식 선택

대부분의 사용자는 `npx`로 시작하는 편이 좋습니다. 현재 프로젝트나 global npm prefix에 이 package를 추가하지 않고 실행할 수 있기 때문입니다.

`npm i`는 `npm install`의 짧은 별칭입니다. 중요한 차이는 `-g`, `-D`, 또는 아무 옵션도 붙이지 않는지입니다.

| 상황 | 명령 | 의미 |
| ---- | ---- | ---- |
| 한 번 써보거나 가끔 실행 | `npx ai-agent-playbook --help` | npm을 통해 해당 명령에 필요한 package를 받아 실행합니다. |
| 어느 디렉터리에서든 `ai-playbook` 사용 | `npm install -g ai-agent-playbook` | global CLI를 설치한 뒤 `ai-playbook --help`로 실행합니다. |
| 한 프로젝트에 도구 버전 고정 | `npm install -D ai-agent-playbook` | 해당 프로젝트의 `node_modules`에 추가하고 `npx ai-playbook ...`으로 실행합니다. |
| 이 저장소 clone에서 직접 실행 | `node .\bin\ai-playbook.mjs --help` | checkout된 source를 직접 실행합니다. |
| 그냥 `npm install ai-agent-playbook` | 보통 첫 선택지는 아님 | 현재 프로젝트에 package를 설치하지만, skill 설치나 `.ai-playbook/` bootstrap은 하지 않습니다. |

npm package 설치와 playbook content 설치는 별도 단계입니다. `npm install`은 도구를 설치합니다. `skills install`은 재사용 스킬을 사용자 skill root에 복사합니다. `bootstrap`은 대상 프로젝트에 playbook을 씁니다.
...
설치 명령은 package 안의 모든 `skills/<category>/<skill>/SKILL.md` 폴더를 일반적인 로컬 스킬 디렉터리에 복사합니다.
...
`skills update`는 반복 실행해도 안전합니다. 관리 중인 스킬을 갱신하고, 기존 matching copy를 adopt하며, 로컬에서 수정된 managed skill은 `--force-managed`가 없으면 거부합니다.

이 playbook이 설치한 managed skill을 제거하려면:
...
`npm uninstall -g ai-agent-playbook`은 global CLI package만 제거합니다. 이미 `%USERPROFILE%\.codex\skills` 또는 `%USERPROFILE%\.agents\skills`에 복사된 skill은 삭제하지 않습니다. 스킬 삭제에는 `skills uninstall`을 사용합니다.
...
Managed project harness 명령은 `.ai-playbook/.ai-agent-playbook-install.json`으로 이 playbook이 복사한 파일을 추적합니다. 정리 전에는 `managed check`를 사용하고, `managed catalog`로 소유 파일을 kind/status별로 검토하고, 기존 matching install에는 `managed adopt`를, 선택한 수정되지 않은 managed file 제거 preview에는 `managed prune`을, 전체 수정되지 않은 managed file 제거 preview에는 `managed uninstall`을 사용합니다. `managed adopt`, `managed prune`, `managed uninstall`은 `--apply`가 있을 때만 파일을 씁니다.
...
| 분류 | 스킬 | 언제 쓰는가 |
| ---- | ---- | ----------- |
| Project | `project-bootstrap` | 새 프로젝트 시작, 기존 저장소 인수, project memory와 루트 agent guidance 설정. |
| Project | `repo-onboarding` | 낯선 저장소에서 architecture, tooling, edit, workflow 답변 전에 먼저 살필 때. |
| Project | `project-doc-system` | 프로젝트 AI 문서, map, runbook, decision, plan, worklog를 만들거나 재정리할 때. |
| Quality | `api-contract-boundary` | frontend/backend contract, DTO, mock, payload, adapter를 구현·디버깅·검토할 때. |
| Quality | `ui-style-policy` | 저장소 UI styling policy를 선택, 문서화, 강제할 때. |
| Quality | `style-quality-review` | UI styling, responsive behavior, layout overflow, visual regression을 검토하거나 개선할 때. |
...
| Legacy | `legacy-general` | 흐름이 불명확하고 결합이 숨겨져 있거나 test/documentation이 약한 legacy code를 유지보수할 때. |
| Legacy | `legacy-risk-check` | shared state, CSS/JS, selector, template, form, API, build, deploy에 영향을 줄 수 있는 legacy 변경 전. |
| Legacy | `legacy-feature-addition` | 주변 architecture를 다시 쓰지 않고 behavior, screen, field, rule, integration을 추가할 때. |
...
| Legacy | `legacy-database-heavy-system` | stored procedure, trigger, view, direct SQL, scheduled job, DB-shaped business rule을 유지보수할 때. |
...
- [설치, 업데이트, 삭제](docs/installation.ko.md): npm/npx 사용, global CLI 설정, skill lifecycle, project bootstrap, cleanup, legacy PowerShell 경로.
