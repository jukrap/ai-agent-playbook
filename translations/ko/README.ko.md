...
이 저장소는 자동 실행 에이전트나 명령 묶음만 모아 둔 저장소가 아닙니다. 기본 방식은 사람이 명시적으로 명령을 실행하고, 미리보기 결과를 확인한 뒤 파일을 쓸지 결정하는 흐름입니다. MCP는 선택 사항이며, AI 앱이 자연어 요청 중 필요한 읽기 전용 점검을 도구 이름으로 호출할 수 있게 해 줍니다.
...
| 프로젝트 템플릿 | 프로젝트에 복사해 둘 루트 에이전트 규칙, 스택별 참고 틀, 현재 사실과 결정 근거를 기록하는 `.ai-playbook/` 문서입니다. | `templates/` |
| 런타임 하네스 | `.ai-playbook/` 생성, 상태 점검, 문맥 수집, 작업 실행 기록, 계약 점검, 계획과 작업 기록 생성을 돕는 명령줄 도구입니다. | `bin/`, `src/` |
| MCP 도구 | AI 앱에서 호출할 수 있는 읽기 전용 도구, 자료, 프롬프트입니다. 스킬 목록, 작업 흐름, 어댑터 준비 상태, 참고 자료 채택 상태, 색인 검색, 쓰기 전 점검, 이미지 차이, 소스 분석 등을 다룹니다. | `src/` |
| 사람용 문서 | 설치, 명령어, 분류, 유지보수, 공개 전 점검, 번역 정책을 설명합니다. | `docs/` |
...
사용 중인 AI 앱이 MCP를 지원한다면 `npx ai-agent-playbook mcp` 같은 로컬 서버 명령을 등록할 수 있습니다. 그러면 명령어를 전부 외우지 않아도 AI에게 프로젝트 문맥 점검, 스킬과 작업 흐름 목록 확인, 어댑터 준비 상태 확인, 참고 자료 채택 상태 확인, 로컬 검색, 심층 분석을 자연어로 요청할 수 있습니다. MCP 도구는 기본적으로 읽기 전용입니다.
...
npm 패키지는 명령줄 도구만 설치합니다. 스킬 복사, `.ai-playbook/` 생성, 훅 활성화, 명령 단축 등록은 자동으로 하지 않습니다. 이 작업들은 사용자가 명시적으로 실행해야 합니다.
...
명령어별 사용법은 [명령어 가이드](docs/commands.ko.md)를 봅니다. 업데이트, 삭제, 로컬 체크아웃, PowerShell 호환 경로, 소유권 표식, 정리 절차는 [설치, 업데이트, 삭제](docs/installation.ko.md)를 봅니다.
...
npx 또는 전역 설치
  ...
  -> 에이전트 재시작
  -> 대상 프로젝트 점검
  -> 프로젝트에 로컬 작업 지침이 필요할 때만 .ai-playbook/ 생성
...
`.ai-playbook/`을 생성한 뒤에는 에이전트가 `START_HERE.md`에서 시작하고, 이어서 `CURRENT.md`, `questions.md`, 관련 기억 문서, 구조 지도, 계약 문서, 작업 흐름 레시피를 읽어야 합니다. `runtime/` 아래 생성 파일은 검토와 승격 전까지 신뢰된 기억이 아니라 근거 후보입니다.
...
- 대상 프로젝트에 `.ai-playbook/` v2 구조를 만들 수 있습니다.
- `START_HERE.md`, `CURRENT.md`, `questions.md`, `SKILLS.md`, `GIT.md`로 에이전트의 시작 지점을 명확히 합니다.
- `memory/`는 사람이 신뢰하는 장기 지식, `runtime/`은 도구가 만든 임시 근거로 분리합니다.
- 기존 `ai-playbook/` 또는 v1 구조도 읽을 수 있고, v2 이전 미리보기를 제공합니다.
...
- 기본 MCP 도구는 읽기 전용입니다.
- 쓰기 가능한 MCP 도구는 `--enable-write-tools`와 호출 시 `apply: true`가 모두 있어야 동작합니다.
...
- 훅은 선택 사항입니다. 중요한 규칙은 `AGENTS.md`와 `.ai-playbook/`에 남기는 것이 기본입니다.
...
```text
bin/                  ai-playbook 명령 진입점
src/                  명령줄 도구와 MCP 서버 구현
skills/
  ai-harness/         MCP, 스킬, 에이전트, 문맥, 사실 점검, 증거 이력, 색인 설계
  architecture/       경계 검토, 기능 단위, 도메인 모델, 모노레포/패키지 구조
  backend/            API 계약, 백엔드 변경 안전성, 연동, 서버 렌더링 흐름
  ...
  project/            부트스트랩, 온보딩, 프로젝트 계획, 문서화, 프로젝트 기억
  quality/            UI 품질, 코드 정리, 가벼운 리뷰
  git/                커밋, PR, push, 작업 기록
  meta/               스킬 작성
  legacy/             레거시 시스템 유지보수
templates/            프로젝트에 복사할 에이전트 지침과 `.ai-playbook/` 템플릿
...
- 프로젝트와 문서: 프로젝트 시작, 저장소 온보딩, 프로젝트 기억, 의사결정 기록, 요구사항 정리, 이슈 계획, 릴리스 노트, 문서 패키지.
- 전달과 검증: Git과 작업 기록, 검증 전략, CI 기준, 불안정한 테스트 대응, 테스트 데이터, 평가 하네스.
- AI 하네스: MCP 표면, 문맥과 기억 설계, 에이전트 작업 분배, 스킬 묶음 관리, 런타임 색인과 캐시, 기능 증거 이력, 행동 전 사실 점검.
- 아키텍처와 백엔드: 경계 검토, 기능 단위 구조, 도메인 모델, 모노레포 패키지, API 계약, 백엔드 변경 안전성, 외부 연동, 서버 렌더링 흐름.
- 데이터와 데이터베이스: 분석, 데이터 계보, 마이그레이션, 검색 지식 기반, 원본 등록, 스키마 변경, 쿼리 성능, 데이터 무결성.
- 운영과 릴리스: 컨테이너, 배포, 패키지 공개, 운영 장애 대응, 릴리스 준비.
- 디자인과 프론트엔드: 디자인 방향, 브랜드 정체성, 참고 화면 분석, 이미지/Figma 인수인계, UI 다듬기, 접근성, 상태와 데이터 흐름, 시각 회귀, 3D 상호작용.
...
- [설치, 업데이트, 삭제](docs/installation.ko.md): npm/npx 사용, 전역 명령 설정, 스킬 관리, 프로젝트 부트스트랩, 정리 절차.
- [런타임 하네스](docs/harness-runtime.ko.md): 런타임 원칙, JSON 계약, 덮어쓰기 정책, 대상 프로젝트 적용 흐름.
- [Harness OS](docs/harness-os.ko.md): 목록, 배치, 런타임, MCP, 스킬을 다시 설계한 원칙.
- [Playbook layout v2](docs/playbook-layout-v2.ko.md): `.ai-playbook` v2 디렉터리 역할과 이전 명령.
- [Skill taxonomy v2](docs/skill-taxonomy-v2.ko.md): 능력 중심 분류와 호환 스킬 정책.
...
영어 원문을 기준으로 유지하고, 영어 원문을 바꾸면 한국어 번역도 같은 변경에서 갱신합니다. 개인 경로, 자격 증명, 내부 URL, 브랜치 이름, PR 번호, 설치된 로컬 스킬 복사본은 커밋하지 않습니다.
