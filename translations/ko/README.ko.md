...
| MCP 도구 | AI 앱에서 호출할 수 있는 읽기 전용 도구, 자료, 프롬프트입니다. 스킬 목록, 작업 흐름, 사용 순서, 어댑터 준비 상태, 참고 자료 채택 상태, 자연스러운 글 점검, 색인 검색, 쓰기 전 점검, 이미지 차이, 소스 분석 등을 다룹니다. | `src/` |
...
- 대상 프로젝트에 구조화된 `.ai-playbook/`을 만들 수 있습니다.
...
- 기존 `ai-playbook/` 경로는 `migrate path`로 `.ai-playbook/`으로 전환할 수 있습니다.
...
- [구조화 플레이북 레이아웃](docs/structured-playbook-layout.ko.md): `.ai-playbook` 디렉터리 역할과 전환 명령.
