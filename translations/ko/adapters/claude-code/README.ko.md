...
- project `AGENTS.md`와 `.ai-agent-playbook/`에 있는 portable document harness;
...
Package publish 뒤에는 `npx ai-agent-playbook`, global install 뒤에는 `aapb`, 로컬 checkout에서는 `node .\bin\aapb.mjs`를 사용합니다. Adapter hook path를 settings에 보관할 때는 렌더링된 hook command가 안정적인 위치를 가리키도록 global install 또는 로컬 checkout을 권장합니다.
...
`settings.example.json`은 renderer만으로 충분하지 않을 때의 수동 참고용으로만 사용합니다. Timeout은 짧게 유지하고, troubleshooting 중에만 `AI_AGENT_PLAYBOOK_DEBUG=1`로 debug output을 stderr에 남깁니다.
...
Project policy는 `.ai-agent-playbook/`에 두고, 재사용 가능한 source content는 이 저장소에 둡니다. Claude Code hook과 skill은 선택적 runtime 편의 기능이며 project memory의 유일한 위치가 아닙니다.
