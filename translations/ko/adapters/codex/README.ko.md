...
Windows용 Codex App에서는 이 저장소 체크아웃을 로컬 기준 원본으로 두고 PowerShell에서 설정을 실행합니다. 대상 저장소 경로에 공백이나 비ASCII 문자가 들어갈 수 있으므로 경로는 변수나 따옴표, `-LiteralPath`로 다룹니다.
...
`templates/codex-home/AGENTS.md`는 첫 번째 계층용입니다. `templates/agents/global/AGENTS.md`는 두 번째 계층용이며 `aapb bootstrap`이 대상 프로젝트에 쓰는 파일입니다. 스킬/Git 정책은 `.ai-agent-playbook/policy/SKILLS.md`와 `.ai-agent-playbook/policy/GIT.md` 아래에 둡니다.
...
스킬을 동기화한 뒤에는 Codex App을 재시작하거나 새 세션을 시작해 스킬 메타데이터가 갱신되게 합니다. 로컬 체크아웃에서는 `node .\bin\aapb.mjs skills install` 또는 호환용 `.\install.ps1` 스크립트를 사용할 수 있습니다.
...
프로젝트별 규칙은 Codex home 전역 파일에 넣지 않습니다. 대상 프로젝트의 루트 `AGENTS.md`는 진입점으로만 쓰고, 저장소 동작은 `.ai-agent-playbook/` 문서에 둡니다.
...
대상 프로젝트에 이미 `AGENTS.md` 또는 `.ai-agent-playbook/`이 있으면 `--force`를 쓰지 말고 충돌을 확인합니다. 더 안전한 시험 경로는 임시 폴더에 발판 파일을 만든 뒤 생성된 파일을 보고 프로젝트에 필요한 부분만 수동 병합하는 것입니다.
...
레거시 또는 문서가 많은 프로젝트에는 보통 `.ai-agent-playbook/START_HERE.md`, `CURRENT.md`, 문서 지도만 먼저 얹습니다. 기존 작업 기록과 계획은 사람이 이전 작업을 검토하기 전까지 제자리에 둡니다.

## 로컬 동기화

새 컴퓨터에서의 전체 설정은 `../../docs/lifecycle.ko.md`를 봅니다. 저장소 루트에서 clone 뒤 한 번 실행합니다.
...
저장소가 공개되거나 접근 가능해진 뒤에는 스킬 관리자가 최종 저장소 URL에서 직접 설치할 수도 있습니다.

```text
<repo-url>
```

비공개 저장소는 대상 도구에서 Git 인증이 필요할 수 있습니다.

## 원본 규칙

로컬 설치 스킬 디렉터리 아래 파일을 기준 원본처럼 수정하지 않습니다. 이 저장소를 수정하고, 검증한 뒤, 동기화합니다.

## 런타임 CLI

이 저장소에는 프로젝트 하네스 설정과 유지보수를 위한 작은 Node CLI도 포함되어 있습니다. 설치형 Codex 스킬은 아닙니다. Package publish 뒤에는 `npx ai-agent-playbook`, global install 뒤에는 `aapb`, 로컬 checkout에서는 `node .\bin\aapb.mjs`를 사용합니다. Adapter hook path를 settings에 보관할 때는 렌더링된 hook command가 안정적인 위치를 가리키도록 global install 또는 로컬 checkout을 권장합니다.
...
대상 프로젝트에 이미 `.ai-agent-playbook/`이 있고 이 checkout의 누락된 가이드 템플릿만 가져오려면 `guides sync`를 사용합니다. 기본값은 기존 가이드 파일을 유지합니다. `guides sync --check --diff --json`은 source와 target hash와 첫 차이 line으로 stale guide file도 보고합니다. 가이드 덮어쓰기를 검토한 뒤에만 `--force`를 사용합니다.

Legacy `ai-playbook/`에서 `.ai-agent-playbook/`로 옮기기 전에는 `migrate path --json`으로 먼저 preview합니다. 이 흐름은 hook 설정과 분리되어 있으며 adapter settings를 설치하거나 편집하지 않습니다.
...
Hook은 스스로 설치되지 않고, project file을 편집하지 않고, tool output을 다시 쓰지 않고, network call을 하지 않습니다. `.ai-agent-playbook/`이 없거나 지원되지 않거나 읽을 수 없으면 stdout 없이 성공 종료합니다.
...
JSON output은 `npx ai-agent-playbook mcp`를 쓰는 MCP settings 예시와 global install용 `aapb mcp` 예시도 함께 포함합니다. AI 앱이 MCP를 지원하면 검토 후 수동으로 복사합니다.
...
`hooks.example.json`은 renderer만으로 충분하지 않을 때의 수동 참고용으로만 사용합니다. Timeout은 짧게 유지하고, troubleshooting 중에만 `AI_AGENT_PLAYBOOK_DEBUG=1`로 debug output을 stderr에 남깁니다.
