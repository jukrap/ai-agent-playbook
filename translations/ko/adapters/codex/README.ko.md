...
- 프로젝트 루트: 대상 프로젝트의 `AGENTS.md`에 두는 저장소 규칙.

`templates/codex-home/AGENTS.md`는 첫 번째 계층용입니다. `templates/agents/global/AGENTS.md`는 두 번째 계층용이며 `ai-playbook bootstrap`이 대상 프로젝트에 쓰는 파일입니다.
...
프로젝트별 규칙은 Codex home 전역 파일에 넣지 않습니다. 저장소 동작은 대상 프로젝트의 루트 `AGENTS.md`와 `ai-playbook/` 문서에 둡니다.
