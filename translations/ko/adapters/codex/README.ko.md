...
스킬을 동기화한 뒤에는 Codex App을 재시작하거나 새 세션을 시작해 스킬 메타데이터가 갱신되게 합니다. 로컬 체크아웃에서는 `node .\bin\ai-playbook.mjs skills install` 또는 호환용 `.\install.ps1` 스크립트를 사용할 수 있습니다.
...
대상 프로젝트에 이미 `AGENTS.md` 또는 `.ai-playbook/`이 있으면 `--force`를 쓰지 말고 충돌을 확인합니다. 더 안전한 시험 경로는 임시 폴더에 발판 파일을 만든 뒤 생성된 파일을 보고 프로젝트에 필요한 부분만 수동 병합하는 것입니다.
...
레거시 또는 문서가 많은 프로젝트에는 보통 `.ai-playbook/START_HERE.md`, `CURRENT.md`, 문서 지도만 먼저 얹습니다. 기존 작업 기록과 계획은 사람이 이전 작업을 검토하기 전까지 제자리에 둡니다.
...
Codex 또는 다른 AI 앱이 로컬 도구를 직접 호출할 수 있다면 MCP를 사용합니다. `npx ai-agent-playbook mcp` 또는 global install 뒤 `ai-playbook mcp`를 local stdio server command로 등록합니다. 이 버전의 MCP 표면은 read-only이며 Codex hook 설정과 별개입니다.
