...
- `skills/ai-harness`: MCP, forge automation, agent, context, fact gate, witness, cache, index, harness design 작업 흐름.
...
- `skills/backend`: API 계약, 백엔드 변경 안전성, 요청/오류 계약, 잡/워커, 연동, 서버 렌더링 흐름 작업 흐름.
...
- `skills/design`: design direction, brand identity, visual reference analysis, image/Figma handoff, visual evidence 작업 흐름.
- `skills/frontend`: UI, 스타일 정책, 브라우저 동작, 상태/데이터, 접근성, 시각 회귀, 디자인 시스템 인수인계, 인터랙티브 미디어 작업 흐름.
...
- `skills/project`: 프로젝트 초기 설정, 온보딩, 요구사항, 이슈 계획, 배포 노트, 자연스러운 글 다듬기, 문서 묶음, 프로젝트 메모리 유지보수.
...
- `templates/project-playbook`: 대상 저장소에서 `.ai-agent-playbook/`가 되는 복사용 프로젝트 메모리 템플릿. 내부 `SKILLS.md`와 `GIT.md` 정책 파일을 포함합니다.

대상 프로젝트 루트는 작게 유지합니다. 런타임은 기본적으로 얇은 루트 `AGENTS.md` 부트스트랩만 쓰고, skill/Git 정책은 `.ai-agent-playbook/` 아래에 둡니다.
...
- `project-bootstrap`: 저장소를 확인한 뒤 루트 정책과 `.ai-agent-playbook/` 구조를 설정합니다.
- `repo-onboarding`: 계획이나 편집 전에 저장소 상태와 기존 `.ai-agent-playbook/` 맥락을 읽습니다.
- `project-doc-system`: `.ai-agent-playbook/`, map, runbook, decision, plan, worklog, archive를 정리합니다.
...
- `natural-writing-humanization`: 한국어 또는 영어 글의 의미, 사실, 기술 용어, 작성 의도를 보존하면서 자연스럽게 다듬습니다.
...
- `agent-orchestration-handoff`: multi-agent worker contract, context budget, evidence ledger, reconciliation gate, handoff를 검토합니다.
- `forge-automation-control`: provider capability 탐지, permission profile, resumable tick, scheduler recovery, Git delivery, local fallback을 제어합니다.
...
- `backend-change-safety`: service, module, job, worker, queue, config, integration, server-side business logic과 evidence-based stack profile selection을 검토합니다.
- `request-validation-error-contract`: 요청 파싱, 검증, 오류 봉투, 예외 매핑, 클라이언트에 보이는 실패 계약을 검토합니다.
- `job-worker-reliability`: 잡, 워커, 큐, 스케줄러, 재시도, 데드레터 경로, 재실행, 오래 실행되는 작업을 검토합니다.
...
- `design-brief-direction`: vague product, page, brand, UI request를 design direction, visual language, constraint, decision-ready brief로 바꿉니다.
- `brand-identity-system`: brand identity, typography, color, logo direction, iconography, voice, application rule을 정의하거나 검토합니다.
- `design-reference-analysis`: upstream visual을 복사하지 않고 screenshot, competitor site, reference app, visual sample, design board를 분석합니다.
- `image-to-code-handoff`: generated image, screenshot, mockup, reference board, Figma frame을 implementation-ready UI contract로 바꿉니다.
...
- `style-policy-selection`: 눈에 보이는 UI 편집이나 오래 남을 스타일 정책 문서화 전에 저장소 스타일링 방식을 선택하거나 조정합니다.
