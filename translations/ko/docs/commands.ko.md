...
기존 이슈나 draft PR을 검토된 표시 구조로 다시 작성해야 할 때는 승인 계획에 `coordination.reconcile.supportingIssues`와 `coordination.reconcile.pullRequests`를 선택적으로 추가할 수 있습니다. 각 항목은 정확한 원격 번호와 충분한 사람용 내용을 가집니다. Reconcile은 계획 전에 최신 `updatedAt`, 제목, draft 상태, branch snapshot을 읽으며, apply는 검토된 그 산출물만 도입하고 drift가 있으면 중단합니다. Preview에는 공개 제목, 본문 완성도와 이슈, Project, View, label, milestone, PR, Project item 수가 표시됩니다. 기존 이슈와 라벨은 삭제하지 않습니다.

GitHub Projects를 우선 사용하도록 설정했지만 현재 `gh` 인증에 `project` scope가 없으면, reconcile은 요청된 Project/View 산출물 수를 보여주되 실행 가능한 operation은 반환하지 않습니다. `gh auth refresh -s project`를 실행한 다음 `aapb forge status .`로 다시 확인합니다. Projects를 의도적으로 사용하지 않을 때만 `--allow-capability-fallback projects,views`로 별도의 milestone 기반 preview를 만듭니다.
