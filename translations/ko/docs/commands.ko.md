...
| `--deep` | `operator analyze`에 AST-grep과 TypeScript/JavaScript 언어 분석 signal을 추가합니다. |
...
| `operator analyze <target>` | diagnostics, map, rules, context, optional local setup signal을 하나의 report로 묶습니다. AST와 언어 분석 signal이 필요하면 `--deep`을 더합니다. | 아니오 | `npx ai-agent-playbook operator analyze <target-project> --deep --path src/example.ts --json` |
...
Text search만으로 부족하고 구조와 언어 signal이 필요하면 `operator analyze --deep`을 사용합니다. Deep mode는 local AST-grep 검색과 TypeScript/JavaScript diagnostics, symbols, references, definitions를 추가합니다. 그래도 파일을 쓰지 않고, project command를 실행하지 않고, symbol rename이나 AST rewrite/apply를 하지 않고, network도 호출하지 않습니다.
...
- QA와 deep analysis: `qa_image_diff`, `ast_grep_search`, `lsp_status`, `lsp_diagnostics`, `lsp_symbols`, `lsp_references`, `lsp_definition`
