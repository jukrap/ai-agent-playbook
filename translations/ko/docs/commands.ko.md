...
`automation resume --reset-attempts`는 blocked task의 retry budget을 다시 허용하는 명시적 결정입니다. 모든 과거 ledger event를 보존하고 `attempt:1` 같은 기존 event ID를 재사용하지 않으며 다음 단조 증가 attempt serial로 계속합니다. Reset 전에 이전 실패 evidence를 확인하고, plan이나 execution baseline에 실제로 별도 이력이 필요할 때만 새 run을 사용합니다.
