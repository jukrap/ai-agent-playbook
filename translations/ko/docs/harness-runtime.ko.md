...
`runtime schema-check`는 target-relative JSON을 파일 쓰기 없이 검증합니다. Generic runtime artifact envelope와 eval definition, eval run report, capability witness, evidence envelope, `.ai-playbook/knowledge/sources.json`용 compact schema를 지원합니다. Compact schema check는 local absolute path, credential처럼 보이는 값, 잘못된 enum 값, non-portable artifact path, oversized inline evidence를 거부하므로 generated report와 source registry entry를 canon 또는 documentation promotion 전에 검토 가능한 상태로 유지합니다.
