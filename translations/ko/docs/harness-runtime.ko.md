...
`aapb`는 재사용 스킬을 설치하고 이 저장소를 대상 프로젝트에 적용하기 위한 실행 명령입니다. 대부분의 inspection/scaffold 명령은 AI 모델을 호출하지 않습니다. 명시적으로 시작한 automation tick은 configured Codex, Claude, argv-based executor를 호출할 수 있고, controller가 결과를 독립적으로 검증하고 기록합니다.

MCP 서버는 AI 앱을 위한 local stdio surface입니다. 기본 tool은 automation status/plan validation과 forge status/plan을 포함해 read-only입니다. 별도 gate를 통과한 forge bootstrap/sync tool만 server를 `--enable-forge-write-tools`로 시작하고 call에 `apply: true`를 전달했을 때 인증된 coordination write를 수행할 수 있습니다. Task execution, push, merge, release는 MCP로 노출하지 않습니다.
...
- `aapb mcp`는 AI 앱용 로컬 표준 입출력 MCP 서버를 시작합니다. 기본 surface는 read-only이고, write surface는 별도 server gate와 call별 apply 승인이 필요합니다.
...
- MCP: 기본 read-only AI tool과 별도로 gate된 bounded playbook/forge coordination write입니다. Automation task나 Git delivery는 실행하지 않습니다.
...
`npm run typecheck`는 TypeScript를 JavaScript 검사 모드로 사용합니다. Automation/forge module과 config resolution, Python bridge, 글 분석, 능력 이력, 의존성 목록, 라우트/API 단서, 심볼 개요 leaf module을 검사합니다. Public CLI, MCP server, adapter, package entrypoint는 계속 JavaScript facade로 둡니다.
...
## Automation 준비 상태와 forge queue 경계

`automation doctor`는 effective policy가 Git operation을 요구할 때 Git `2.39.0+`를 요구합니다. GitHub read path가 탐지되고 GitHub CLI가 설치되어 있으면 `2.80.0+`여야 합니다. GitHub Projects scope 누락은 warning이며 authentication refresh를 실행하지 않습니다. 설치된 Gitea `tea`가 `0.14.2`보다 낮을 때도 documented REST fallback을 사용할 수 있으므로 warning으로 보고합니다.

Doctor는 user checkout의 dirty 상태와 unattended execution의 isolation 여부를 보고합니다. Dirty user checkout은 `git.unattendedWorkspace: "isolated-checkout"`일 때만 허용됩니다. `--no-git`을 포함한 unattended work는 committed Git baseline에서 만든 별도 checkout을 사용해 dirty/untracked user file을 제외하며, non-Git unattended target은 거부합니다. Dirty non-isolated checkout은 conflict입니다. Scheduler readiness는 local executable availability 또는 hosted provider/repository compatibility만 나타냅니다. Schedule 등록, Actions 활성화, runner 상태, credential 동작, 실제 remote write 성공을 증명하지 않습니다.

Remote read가 허용되면 `automation start`가 configured ready label이 있는 open non-pull-request issue를 찾고 closed 또는 paused issue를 제외한 뒤 approved local task를 덮어쓰지 않고 merge합니다. 이 탐색은 새 run을 만들 때와 같은 non-terminal run을 재사용할 때 모두 수행하며, 새 eligible issue는 run lease 아래 멱등 append합니다. Remote title, body, criterion은 untrusted data입니다. Remote command와 path는 채택하지 않고, queue issue는 검토한 local execution mapping이 trusted path와 verification argv를 제공할 때까지 pause합니다. `--no-remote`와 `--offline`은 이 조회를 완전히 건너뜁니다.

Linked issue가 있으면 forge transport를 사용할 수 있는 tick이 claim 전과 executor 작업 후에 remote state를 inspect합니다. Configured pause label, ready label 제거, issue close는 task 또는 run을 pause합니다. Claim 전 requirement drift는 아직 claim되지 않은 task에 import할 수 있고, executor 작업 후 drift는 verification이나 delivery 전에 `needs-reconcile`로 pause합니다. Remote inspection을 사용할 수 없으면 이 guard는 remote state를 판정할 수 없으며 local policy가 계속 실행할지를 결정합니다. `forge reconcile`은 기본으로 preview하고 mutation을 차단한 provider preflight로 확인된 no-op과 실행 가능한 변경을 분리합니다. `--run-id <id> --apply`는 eligible pre-claim import 또는 reconciliation pause를 schema v2 ledger에 기록할 수 있지만 run을 조용히 approve하거나 resume하지 않습니다.
...
`workflow run-preview`는 target-local recipe를 먼저 읽고, 없으면 bundled recipe로 fallback하며, run contract를 parse한 뒤 파일을 쓰지 않고 generated evidence를 반환합니다.

`workflow run-start`는 project-write가 아니라 scaffold tier에 속합니다. 명시적 `--apply`가 있을 때만 `.ai-agent-playbook/workflows/runs/` 아래에 bounded run manifest, criteria checklist, evidence notes stub, handoff stub을 씁니다. Missing recipe, empty manifest, path traversal, project-source destination, trusted memory destination, unsafe overwrite를 거부합니다.
...
기존 `run start/status/record/summarize` surface는 수동으로 기록하는 진행 중 상태와 evidence를 다룹니다. Schema v2 automation controller 및 `worklogs/`와 역할이 다릅니다.
