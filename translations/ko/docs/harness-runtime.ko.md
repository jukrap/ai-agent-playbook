...
`config preview`는 Harness OS 기본값을 read-only로 해석하는 resolver입니다. 최종 값, source map, source file 상태, warning, conflict를 보고하지만 runtime behavior를 바꾸거나 파일을 쓰지 않습니다.
...
Path-scoped context는 `.ai-playbook/context/`에 둡니다. Context markdown은 `id`, `globs`, `alwaysApply`, `freshness`, `priority` frontmatter를 사용할 수 있고, 본문에는 `When to read`, `Current facts`, `Do not assume`, `Verification hints` 같은 섹션을 권장합니다.

`context list`와 `context status`는 read-only입니다. `context status --path <file> --json`은 `{ schemaVersion, ok, target, path, summary, contexts, docMap, warnings, conflicts }`를 반환해 특정 path에 적용되는 context file과 `.ai-playbook/maps/doc-map.md` 존재 여부를 보여줍니다. `context init`은 preview-first이며 `--dry-run`을 빼야 starter context와 documentation map을 씁니다.
...
- Runs는 `.ai-playbook/runs/<run-id>/`에 생성됩니다.
- Contract starter는 `.ai-playbook/contracts/`에 생성됩니다.
