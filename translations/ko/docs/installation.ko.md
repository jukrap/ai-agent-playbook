...
1. npm package는 `ai-playbook` CLI와 bundled source file을 설치합니다.
2. `skills install`은 재사용 skill을 사용자 수준 skill root에 복사합니다.
3. `bootstrap`은 대상 repository 하나에 project playbook을 복사합니다.

npm package 설치만으로는 skill 복사, `.ai-playbook/` 생성, hook 활성화, slash command 등록이 일어나지 않습니다. 그런 작업은 명시적으로 실행해야 합니다.

## CLI 설치 방식 선택

Node.js를 사용할 수 있을 때 사용합니다. Public package는 [`ai-agent-playbook`](https://www.npmjs.com/package/ai-agent-playbook)입니다.

`npm i`는 `npm install`의 짧은 별칭입니다. Package가 어디에 들어가는지는 scope option이 결정합니다.

| 목표 | 명령 | 결과 |
| ---- | ---- | ---- |
| 도구를 시험하거나 가끔 실행 | `npx ai-agent-playbook --help` | npm이 해당 명령에 필요한 package를 받아 실행합니다. 프로젝트 dependency는 추가하지 않습니다. |
...
그냥 `npm install ai-agent-playbook`을 일반적인 첫 단계처럼 쓰지는 않는 편이 좋습니다. 현재 프로젝트의 runtime dependency처럼 추가하고 싶을 때만 사용합니다. 이 명령은 현재 프로젝트의 `node_modules`에 설치하지만, 그래도 skill 설치나 project playbook bootstrap은 하지 않습니다.
...
Skill 설치 뒤에는 새 skill metadata를 읽도록 Codex를 재시작하거나 새 agent session을 시작합니다.
...
Global install 후에는 아래 예시의 `npx ai-agent-playbook`을 `ai-playbook`으로 바꿔 실행할 수 있습니다.

## 스킬 lifecycle

재사용 skill은 대상 repository가 아니라 사용자 수준 skill root에 설치됩니다.
...
Managed local skill 설치 또는 업데이트:
...
`skills install`과 `skills update`는 반복 실행해도 안전합니다. managed skill을 일반 Codex/agent skill directory로 동기화합니다. 로컬에서 수정한 managed skill은 `--force-managed`가 없으면 덮어쓰지 않고, 같은 이름의 다른 unmanaged skill은 `--force-unmanaged`가 없으면 덮어쓰지 않습니다.

`skills uninstall`은 이 playbook이 설치한 수정되지 않은 managed skill만 제거합니다. 먼저 `--dry-run`으로 확인합니다. Managed skill을 로컬에서 수정했다면, 유지할지 아니면 `--force-managed`로 의도적으로 지울지 결정할 때까지 중단합니다.

Ownership marker는 각 설치된 skill folder 안의 `.ai-agent-playbook-install.json`입니다. 이 marker에는 `source: "ai-agent-playbook"`과 hash가 들어 있어 CLI가 이 playbook의 managed copy와 다른 사람이 만든 skill을 구분합니다.

Skill 설치나 업데이트 뒤에는 다음 세션이 skill metadata를 읽도록 Codex를 재시작합니다.

## Global CLI lifecycle

Global npm package와 복사된 skill은 서로 다릅니다. Global package를 삭제해도 이미 복사된 skill은 삭제되지 않습니다.
...
복사된 skill을 제거하려면 `npx ai-agent-playbook skills uninstall` 또는 `ai-playbook skills uninstall`을 사용합니다.
...
| `npm install -g ai-agent-playbook` | 예 | npm global package 위치만 변경합니다. |
| `npm install -D ai-agent-playbook` | 예 | 현재 프로젝트의 `package.json`, lockfile, `node_modules`를 변경합니다. |
| `skills check` | 아니오 | skill 상태를 보고합니다. |
| `skills install` / `skills update` | `--dry-run`이 없으면 예 | 사용자 skill root를 변경합니다. |
| `skills uninstall` | `--dry-run`이 없으면 예 | 사용자 skill root에서 managed skill을 제거합니다. |
...
템플릿은 skill처럼 자동 설치되지 않습니다. Skill은 사용자 수준 재사용 guidance이고, project playbook은 repository별 project memory입니다. 권장 경로는 runtime CLI이며, 더 세밀한 제어가 필요할 때만 수동으로 복사하거나 조정합니다.
