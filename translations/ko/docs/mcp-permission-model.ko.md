...
- `ai-agent-playbook://capabilities`: capability catalog와 coverage summary.
- `ai-agent-playbook://skills`: skill taxonomy와 compatibility wrapper metadata.
- `ai-agent-playbook://workflows`: bundled workflow recipe.
- `ai-agent-playbook://adapters`: Codex, Codex App, Claude Code, MCP 설정 요약.
- `ai-agent-playbook://adapter-readiness`: adapter check/config 명령, readiness check, no-write boundary.
- `ai-agent-playbook://agent-usage-guide`: resource, prompt, read-only 도구를 고르는 짧은 사용 순서 안내.
- `ai-agent-playbook://playbook-layout`: 구조화된 `.ai-agent-playbook` layout 역할과 권장 읽기 순서.
- `ai-agent-playbook://reference-adoption`: reference registry, ledger, status, promotion boundary 요약.
- `ai-agent-playbook://mcp-permission-model`: 이 권한 모델의 structured JSON.
...
Scaffold-tier와 managed-write tool은 기본으로 노출하지 않습니다. AI 앱이 opt-in write tool을 봐야 할 때만 local server를 `aapb mcp --enable-write-tools`로 시작합니다.

현재 opt-in write tool은 다음입니다.

- `workflow_run_start`: `.ai-agent-playbook/workflows/runs/` 아래 workflow run file을 미리 보거나 생성합니다.
- `write_gate_advisory`: `.ai-agent-playbook/runtime/reports/write-gate/` 아래 runtime advisory를 미리 보거나 저장합니다.
- `reference_ledger_update`: `.ai-agent-playbook/knowledge/reference-adoption-ledger.md`에 누락된 row를 미리 보거나 추가합니다.
- `reference_ledger_decision`: 기존 reference adoption ledger row 하나의 decision을 미리 보거나 갱신합니다.
- `reference_source_registry_update`: `.ai-agent-playbook/knowledge/sources.json`에 누락된 source entry를 미리 보거나 추가합니다.
