...
**구조:** CLI와 `.ai-agent-playbook/` 문서 하네스를 안정 core로 유지합니다. Runtime 자동화는 diagnostics, local configuration helper, 제한적 blocking/continuation 실험 순서로만 추가합니다. 각 runtime 동작은 이 계획에서 명시하지 않는 한 기본값으로 꺼져 있어야 합니다.
...
- `AI_AGENT_PLAYBOOK_HOOK_EVENTS`를 통한 opt-in `UserPromptSubmit`, `PostToolUse` lifecycle reminder
...
- `.ai-agent-playbook/worklogs/YYYY-MM/*.md` 파일은 있는데 대응하는 `.ai-agent-playbook/worklogs/summaries/YYYY-MM.md`가 없는 month를 감지합니다.
...
- `AI_AGENT_PLAYBOOK_HOOK_EVENTS`를 통해 `Stop` event를 opt in으로 추가합니다.
- Non-blocking으로 유지합니다.
- Continuation을 요청하지 않습니다.
- Doctor를 자동 실행하지 않습니다.
- 대상에 `.ai-agent-playbook/`이 있을 때만 짧은 reminder를 출력합니다.
...
1. `Stop`을 `AI_AGENT_PLAYBOOK_HOOK_EVENTS`의 optional reminder event로 추가합니다.
...
V7은 선택 사항입니다. Full plugin보다 작게 유지합니다. Adapter-local package shell entrypoint가 실제 setup과 smoke-test 마찰을 줄일 때만 추가합니다.

### V7 목표

- 같은 hook, config, check command를 얇은 runtime-specific shell 뒤에 노출합니다.
- Project rule은 plugin-only file이 아니라 `AGENTS.md`와 `.ai-agent-playbook/`에 둡니다.
- Adapter-specific file은 `adapters/` 아래에 둡니다.
- Plugin installation과 사용자 설정 변경은 범위 밖에 둡니다.
...
- Hook, config, check command용 shell smoke test.
- Shell이 선택 사항임을 문서화.
...
- Shell이 유용하다는 점이 확인되기 전까지 full plugin package를 만들지 않습니다.
