...
| 구성 | 뜻 | 위치 |
| --- | --- | --- |
| CLI 패키지 | `aapb` 명령을 제공하는 프로그램 | 소스 폴더, npm 설치 위치 또는 npm 캐시 |
...
전역 설치를 하면 일반 터미널에서 `aapb`를 사용할 수 있습니다. 도움말에서 기록·스킬·선택형 명령을 확인하세요. 전역 설치 없이 가끔 사용하려면 `aapb` 대신 `npx ai-agent-playbook`을 쓸 수 있습니다.
...
aapb records status . --json
aapb records read . --path CURRENT.md
aapb records search . --query "연습 폴더" --json
...
| `aapb` 명령을 찾지 못함 | npm 전역 설치 후 터미널 다시 열기. npm 설치 위치와 PATH를 확인하거나 격리 Node 진입점 사용 |
...
| 용어 | 뜻 |
| --- | --- |
| `aapb` | `ai-agent-playbook` 패키지가 제공하는 실행 명령 |
