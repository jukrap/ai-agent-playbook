...
패키지를 이식 가능한 프로젝트 기록, 선택한 문서 규격, 과제별 전문 기능에 집중시킵니다. Node ESM 구현, 패키지명, `aapb` 명령을 유지합니다. 다양한 작업 절차를 강제하던 진입점을 필요한 스킬과 선택형 참고 자료로 재구성합니다. 모든 외부 스킬이 모든 모델에 해롭다는 전제로 진행하지 않습니다.
...
설계 조사 시점의 자료를 확인했고, 공개 main을 실제 계정에 기능이 제공된 상태으로 간주하지 않고 설치된 Codex 0.153.4 계열과 비교했습니다.

## 앱의 기본 문맥 기능과 비용

[버전별 모델 설정 코드](https://github.com/openai/codex/blob/rust-v0.153.4/codex-rs/models-manager/src/model_info.rs)는 사용자가 지정한 문맥 크기를 지원 최대값으로 제한합니다. [모델 프로토콜](https://github.com/openai/codex/blob/rust-v0.153.4/codex-rs/protocol/src/openai_models.rs)은 설정값, 사용 가능 범위, 자동 압축 한도를 구분합니다.

[history/notes 확장](https://github.com/openai/codex/blob/rust-v0.153.4/codex-rs/ext/history-notes/src/extension.rs)은 설정·제공자·백엔드 인증을 확인하며 대화 힌트를 4,000바이트로 제한합니다. 소스와 모델 기본 활성화 경로가 존재해도 실제 세션 도구 노출까지 증명하지는 않습니다. 사용할 수 없으면 파일 기록으로 진행하며 실험 설정을 필수로 요구하지 않습니다.
...
폐기한 진입점을 호환용 스킬로 다시 설치하지 않습니다. 고유한 참조는 명시적으로 선택하는 참고 자료 모음에 이전 위치와 함께 남깁니다. 참고 자료 모음는 시작 시 전부 읽는 목록이 아닙니다.

## 런타임 결정

- 최소 bootstrap은 CURRENT.md와 소유권·구조를 나타내는 관리 정보를 만들며 빈 정책 문서 묶음을 생성하지 않습니다.
- 기존 구조화 기록과 레거시 기록을 계속 읽습니다. 이전은 관리 파일만 변경하고 수정본·미관리 충돌과 원문을 보존합니다.
...
- 작업 실행기·감독 루프·예약·자동 Git 전달과 중복 분석 명령은 종료합니다. 대체 기능과 고정 0.5.11 복구를 안내하며 구버전을 자동으로 대신 실행하지 않습니다.
...
기준선 테스트는 458개 통과, 1개 건너뜀, 실패 0개였으며 구문·타입·스킬·번역·공개 문서·Python·설치 미리보기가 통과했습니다. 새 구현을 검증한 결과는 아닙니다.
...
지침 축소는 스킬을 찾는 데 필요한 내용과 불필요한 필수 읽기에 적용합니다. 초보자 안내, 명령 설명, 예시, 복구, README의 브랜드는 다른 목적을 가집니다. 기능을 갱신하더라도 독자가 따라갈 경로와 언어별 표현을 보존합니다. [유지보수](maintenance.ko.md)와 [번역 정책](translation-policy.ko.md)을 참고하세요.
