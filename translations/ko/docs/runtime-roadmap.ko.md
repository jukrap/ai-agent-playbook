...
- `migrate path`는 legacy `ai-playbook/`에서 `.ai-playbook/`로의 폴더 이동, 참조 갱신, `.gitignore` 전환을 preview하고 필요할 때만 적용합니다.
- `plan`, `worklog`, `worklog summarize`는 active plan, 상세 이력, 월간 summary를 예측 가능한 경로에 둡니다.
- `managed check`, `managed adopt`, `managed uninstall`은 project-level marker를 사용해 이 playbook이 복사한 수정되지 않은 파일만 확인, 채택, 제거합니다.
- `operator check`, `operator search`, `operator context`, `operator map`, `operator audit`, `operator gc`, `rules check`, `diagnostics check`, `qa tui-check`는 combined health review, local search, path-scoped context preview, codebase mapping, playbook drift audit, preview-first managed cleanup, rule matching, verification command discovery, terminal/CJK layout evidence를 위한 operator-triggered diagnostics를 제공합니다.
...
- Cleanup 전에는 `managed check`를 사용하고, managed removal 전에는 `managed uninstall --json`을 사용합니다.
- 더 강한 runtime automation을 검토하기 전에 `operator check --path`, `operator search --query`, `operator context --path`, `operator map`, `operator audit`, `operator gc`, `rules check --path`, `diagnostics check`, `qa tui-check`를 operator-visible evidence로 사용합니다.
...
- **Session hooks:** `SessionStart`는 native agent context만으로 부족할 때 `.ai-playbook/`의 작은 project reminder를 불러올 수 있습니다. `UserPromptSubmit`은 opt-in으로 유지하며 handoff-like prompt에 짧은 guardrail reminder만 출력합니다.
...
- 두 wrapper 모두 shared `context` core를 호출하며, project file을 편집하지 않고, network call을 하지 않고, `.ai-playbook/`이 없으면 조용히 빠집니다.
- 예시 hook 설정은 각 adapter 옆에 있으며 자동 설치되지 않습니다.
- `adapter config`는 이 checkout의 hook path를 사용해 검토 가능한 local settings를 렌더링하며, 파일을 쓰지 않고 `.ai-playbook/`이 먼저 있어야만 동작하지도 않습니다.
...
- `AI_PLAYBOOK_HOOK_EVENTS`로 `UserPromptSubmit`, `PostToolUse`, `Stop` reminder를 opt in할 수 있습니다. 관련 없는 prompt, missing playbook, unsupported payload, non-edit tool에서는 조용히 빠집니다.
...
- Managed marker, combined operator check, local search, path-scoped context preview, codebase map summary, playbook drift audit, preview-first managed cleanup, rule matching, diagnostics command discovery, TUI layout check가 hook-driven diagnostics로 승격하기 전에 review miss를 줄이는지 확인.
