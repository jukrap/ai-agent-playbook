...
- 더 강한 runtime automation을 검토하기 전에 `operator check --path`, `operator search --query`, `operator context --path`, `operator analyze --path`, `operator map`, `operator audit`, `operator gc`, `rules check --path`, `diagnostics check`, `qa tui-check`를 operator-visible evidence로 사용합니다.
