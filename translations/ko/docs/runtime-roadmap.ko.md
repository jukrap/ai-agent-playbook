...
- **Session hooks:** `SessionStart`는 native agent context만으로 부족할 때 `ai-playbook/`의 작은 project reminder를 불러올 수 있습니다. `UserPromptSubmit`은 이후 후보입니다.
...
- 명시적 opt-in configuration으로 보호되는 `UserPromptSubmit` 또는 `PostToolUse` lifecycle reminder hook.
