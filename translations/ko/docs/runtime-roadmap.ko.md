...
- `doctor --json`, `guides sync --check --json`, `context --json`, `adapter check --json`은 향후 adapter가 읽을 수 있는 작은 machine-readable core를 제공합니다.
...
- `guides sync --check`가 누락뿐 아니라 stale guide도 보고할 수 있도록 guide manifest 또는 version marker 추가.
- Durable fact 승격을 상기시키는 worklog summary freshness check.
- 전체 check를 자동 실행하지 않는 `doctor` reminder hook.
- V4 후보: 비용과 소음이 검증된 뒤의 `Stop`, continuation, blocking feedback, 자동 doctor 실행.
