...
- `guides sync`는 기본적으로 local edit을 교체하지 않고 누락된 guide template만 추가하며, `guides sync --check --json`은 source와 target hash로 stale guide를 보고합니다.
...
- `doctor --json`, `doctor --reminder --json`, `guides sync --check --json`, `context --json`, `adapter check --json`은 향후 adapter가 읽을 수 있는 작은 machine-readable core를 제공합니다.
...
- `AI_PLAYBOOK_HOOK_EVENTS`로 `UserPromptSubmit`과 `PostToolUse` reminder를 opt in할 수 있습니다. 관련 없는 prompt, missing playbook, unsupported payload, non-edit tool에서는 조용히 빠집니다.
...
- 기존 프로젝트를 위한 bootstrap conflict report 개선.
- 파일을 쓰지 않고 검토 가능한 설정을 출력하는 local adapter configuration renderer.
- 아직 신중해야 하는 V4 후보: 비용과 소음이 검증된 뒤의 `Stop`, continuation, blocking feedback, 자동 doctor 실행.
