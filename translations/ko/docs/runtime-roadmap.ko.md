...
- `operator check`, `rules check`, `diagnostics check`, `qa tui-check`는 combined health review, rule matching, verification command discovery, terminal/CJK layout evidence를 위한 read-only operator diagnostics를 제공합니다.
...
- 더 강한 runtime automation을 검토하기 전에 `operator check --path`, `rules check --path`, `diagnostics check`, `qa tui-check`를 operator-visible evidence로 사용합니다.
...
- Combined operator check, rule matching, diagnostics command discovery, TUI layout check가 hook-driven diagnostics로 승격하기 전에 review miss를 줄이는지 확인.
