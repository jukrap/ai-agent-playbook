...
route, DTO, SDK, event, webhook, 클라이언트에 보이는 payload가 호환성 영향을 받을 수 있으면 `references/api-versioning-compatibility.md`를 읽습니다.

영속성, 외부 부수 효과, event, job, retry의 일관성이 중요하면 `references/transaction-side-effect-boundary.md`를 읽습니다.

configuration, feature flag, cache, process startup, runtime default를 바꿀 때는 `references/config-cache-runtime-contracts.md`를 읽습니다.
