...
저장소가 실제로 사용할 수 있는 project memory와 얇은 root bootstrap만 만듭니다.
...
3. `.ai-agent-playbook/`을 commit할지 local-only로 둘지 결정하고, private note를 쓰기 전에 그 policy를 기록합니다.
4. `templates/agents/global`의 얇은 루트 `AGENTS.md`와 `templates/project-playbook`의 project memory templates를 조정합니다.
