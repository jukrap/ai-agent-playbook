...
- 같은 관심사에 hook을 추가하기 전에 `operator check`, `operator search`, `operator context`, `operator map`, `rules check`, `diagnostics check`, `qa tui-check`를 operator-visible diagnostics로 먼저 사용합니다.
