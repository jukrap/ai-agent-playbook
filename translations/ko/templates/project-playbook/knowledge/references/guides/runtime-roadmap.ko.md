...
- Business rule과 invariant signal은 `contracts check --path`로 read-only 확인합니다.
- Detailed history는 `workflows/worklogs/`에 두고 durable fact가 있으면 summary를 만듭니다.
...
- 같은 관심사에 hook을 추가하기 전에 `context status`, `run status`, `contracts check`, `operator check`, `operator search`, `operator context`, `operator analyze`, `operator map`, `operator audit`, `operator gc`, `rules check`, `diagnostics check`, `qa tui-check`를 operator-visible diagnostics로 먼저 사용합니다.
...
- Native context가 부족할 때 `START_HERE.md`, `CURRENT.md`, `policy/SKILLS.md`, `policy/GIT.md` 기반 compact project-memory context를 주입합니다.
