...
- Approved plan task와 `aapb:ready` label이 있는 기존 issue만 queue에 들어갈 수 있습니다.
...
Doctor는 effective policy가 Git을 요구할 때만 Git `2.39.0` 미만을 conflict로 봅니다. GitHub read path가 탐지되고 설치된 `gh`가 `2.80.0` 미만이면 conflict이며, 설치된 Gitea `tea`가 `0.14.2` 미만이면 documented REST를 사용할 수 있으므로 warning입니다. GitHub Projects scope 누락도 warning이며 `gh auth refresh`를 실행하지 않습니다.
